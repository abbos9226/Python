...
class StudentCRUDSystem:
    ...
    def save_data(self):
        with open('save_students.txt', 'w') as file:
            for student in self.students:
                file.write(f"{student.student_id},{student.name},{student.age},{','.join(map(str, student.grades))},{student.average_grade():.2f}")
        print('Successfully saved!')
    
    def load_data(self):
        with open('save_students.txt','r') as file:
            self.students=[]
            for line in file:
                ...
                name=data[1]
                age=int(data[2])
                grades=list(map(int, data[3:-1])) #Convert grades back to a list of integers
...
def main():
    ...
    while True:
        ...
        if command =='1':
            ...
            student_id = input("Enter Student ID that you want to Save: ")
            crud.save_data()
        elif command=='6':
            crud.load_data() 
        elif command =='7': 
